...
const WAYBACK_SUPPORT = process.env.MEMENTO_WAYBACK_REPOSITORY ?? false;
...
program
  ...
  .option('--wayback','wayback support',WAYBACK_SUPPORT)
  .action( (options) => {
    if (!options['wayback']) {
      ...
      console.log("**wayback repository**");
    }
...
program
  ...
  .option('--sleep <seconds>','sleep',10)
  .option('--attempts <number>', 'how often we need to try to find a status update',10) 
  .option('--wayback','wayback',WAYBACK_SUPPORT)
  .argument('<box>','box to process')
  .action( async(box,options) => {
    if (options['wayback'] && ! (process.env.S3_ACCESS_KEY && process.env.S3_SECRET_KEY)) {
