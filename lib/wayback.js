...
async function _waybackCapture(url,options) {
    ...
    options['attemps'] = options['attemps'] || MAX_ATTEMPTS;
    ...
    return new Promise( async (resolve,reject) => {
        ...
        try {
            ...
            if (post_response && post_response['job_id']) {
                ...
                logger.error(`no job_id generated for ${url}`);
                throw new Error(`no job_id generated for ${url}`);
            }

            let attempt = 0;
            while (attempt < options['attemps']) {
                const status_response = await waybackGET(job_id,options);

                if (! status_response) {
                    logger.error(`no status response for job_id ${job_id}`);
                ...
                if (!status) {
                    logger.error(`no status for job_id ${job_id}`);
                    ...
                    logger.debug(`> ${status}`);
                    logger.error(`error status for job_id ${job_id}`);
                    ...
                    logger.error(`unknown status ${status} for job_id ${job_id}`);
        ...
        if (isHappy) { 
            ...
            logger.error(`no result for ${url} after ${options['attemps']} attemps`);
            resolve(null);
        }
    });
}
