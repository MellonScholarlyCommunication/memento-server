...
async function listMementosFor(url,datetime) {
...
async function getMementoMetadata(url,datetime) {
    const memento = await listMementosFor(url,datetime);

    if (! memento) {
        return null;
    }
    
    const hash = memento['.git']['hash'];
    ...
    const repositoryPath = './mementos/' + checksum + '.json';
    ...
    try {
        ...
        logger.error(e);
        logger.warn(`failed to retrieve ${url} metadata ; rev = ${hash}`);
        return null;
    }
}

async function getMemento(url,datetime) {
    const memento = await listMementosFor(url,datetime);
    ...
    try {
        ...
        logger.error(e);
        logger.warn(`failed to retrieve ${url} ; rev = ${hash}`);
        return null;
    }
}

async function storeMemento(url, original, actor) {
    const repository = process.env.MEMENTO_REPOSITORY;
    ...
    fs.writeFileSync(repositoryPath,original['body']);
    ...
    if (isNewFile) {
        ...
        await git.commit(`offer from ${actor}`, [ `${repositoryPath}.json` ]);
    }
